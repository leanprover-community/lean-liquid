...
begin
  ...
  simp only [preadditive.sum_comp, fin.sum_univ_succ, fin.default_eq_zero,
    category_theory.category.comp_id, add_zero, fin.coe_zero, fin.sum_univ_zero,
    fin.coe_one, one_gsmul, category_theory.eq_to_hom_refl, category_theory.functor.comp_map,
    fin.coe_succ, category_theory.op_comp, neg_gsmul, pow_one, fin.succ_zero_eq_one,
    category_theory.category.assoc, category_theory.functor.right_op_map,
    category_theory.functor.map_comp, pow_zero, finset.sum_congr,
    category_theory.preadditive.add_comp, category_theory.preadditive.neg_comp],
  delta contracting_homotopy,
  simp_rw [← M.map_comp],
  rw ← add_zero (𝟙 _),
  swap, apply_instance,
  ...
  { rw neg_add_eq_zero,
    ...
    ext, swap, simp [splitting],
    simp only [category_theory.category.comp_id,
      quiver.hom.unop_op, category_theory.limits.wide_pullback.π_base,
      category_theory.cech_obj_map, dif_ctx_congr,
      category_theory.category.assoc, category_theory.limits.wide_pullback.lift_π],
    split_ifs with h1 h1,
    { refl },
    { refine false.elim (h1 $ equiv.ulift.injective _),
      change fin.succ_above _ j.down = 0,
      have hjz : j.down = 0, by tidy,
      rw [hjz, fin.succ_above_below],
      { simp },
      { exact fin.lt_succ } } }
end
...
-- LCC = Completeion of LocallyConstant
-- X -> B surjective arrow of Propfinite sets, writ eit as a limit X_i -> B_i of surjective arrows of finite sets.
-- They all split B_i -> X_i.
-- cech_nerve (X -> B) = limit of cech_nerve (X_i -> B_i)
-- complex (LocallyConstant ((cech_nerve (X_i -> B_i)))) = exact with precise control on norms of lifts
-- rescale_complex (LocallyConstant ((cech_nerve (X_i -> B_i)))) = exact with precise control on norms of lifts norm of the n-th splitting
-- something like (n+1)!/n!
-- complex (LocallyConstant ((cech_nerve (X -> B))) = colimit complex (LocallyConstant ((cech_nerve (X_i -> B_i))))
-- LocallyConstant.obj M : Profiniteᵒᵖ ⥤ NormedGroupNoninc
