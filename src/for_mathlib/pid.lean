import for_mathlib.data.finset
import for_mathlib.data.finsupp
import linear_algebra.dimension
import linear_algebra.free_module.basic
import ring_theory.principal_ideal_domain
import set_theory.ordinal.arithmetic

open set submodule submodule.is_principal
...
lemma support_remove {i : ordinal} {j_good : {i // a i ≠ 0}} (hj: j_good.1.1.succ = i)
  {l l': {i // a i ≠ 0} →₀ R}
  (hsupp: (l.support : set {i // a i ≠ 0}) ⊆ {x : {i // a i ≠ 0}) | x.1.1 < i})
...
lemma lin_ind_limit {BB : {i // a i ≠ 0} → M} {i : ordinal} (hio : i ≤ o) (hlim : i.is_limit)
  (H : ∀ k < i, k ≤ o → ∀ (l : {i // a i ≠ 0} →₀ R),
    ((l.support : set {i // a i ≠ 0}) ⊆ {x : {i // a i ≠ 0}) | x.1.1 < k}) →
  ...
  l = 0 :=
begin
  obtain hemp | hemp := l.support.eq_empty_or_nonempty,
  { exact finsupp.support_eq_empty.1 hemp },
  let m := l.support.max' hemp,
  let good := {i  // a i ≠ 0},
  dsimp at hsupp,
  have hm₀ : ↑↑m < i := by exact_mod_cast hsupp (finset.max'_mem l.support hemp),
  have hm : m.1.1.succ < i := (ordinal.succ_lt_of_is_limit hlim).2 hm₀,
  refine H m.1.1.succ hm (hm.le.trans hio) l (λ x hx, _) hl,
  exact order.lt_succ_iff.2 (finset.le_max' l.support x hx),
end

-- Auxiliary lemma for proving the span property at limit ordinals.
lemma span_limit {N : submodule R M} {BB : {i // a i ≠ 0} → M} {i : ordinal}
  (hio : i ≤ o)
  (hlim : i.is_limit)
  {B : basis {i // i < o} R M}
...
begin
  ...
  refine H m.1.succ hm hmo l (λ x hx, _) hl,
  exact order.lt_succ_iff.2 (finset.le_max' l.support x hx),
end

-- Auxiliary lemma for proving linear independence at successor ordinals.
lemma lin_ind_succ [is_domain R] {N : submodule R M} {B : basis {i // i < o} R M}
  {BB : {i  // a i ≠ 0} → N}
...
begin
  ...
  { -- Case: j is good
    ...
    { let l' := l + finsupp.single j_good (- l j_good),
      ...
      { simp [l', finsupp.single_neg, map_add_neg, map_neg] },
    ...
    { have hsub : (l.support : set good) ⊆ (λ x : good, x.1.1 < j),
      ...
      exact H j hji hjo.le l hsub hl } },
  { -- Case: j is not good
    have hsub : (l.support : set good) ⊆ (λ x : good, x.1.1 < j),
    intros x hx,
    have hx : x ∈ l.support := hx,
...
lemma span_succ {N : submodule R M} {BB : {i // a i ≠ 0} → M} {i : ordinal} (hio : i ≤ o)
  (hlim : ¬i.is_limit) {B : basis {i // i < o} R M}
  (H : ∀ (k : ordinal), k < i → k ≤ o → ∀ (l : {i // i < o} →₀ R),
      (l.support : set {i // i < o}) ⊆ {x | x.1 < k} →
      finsupp.total {i // i < o} M R B l ∈ N →
      finsupp.total {i // i < o} M R B l ∈ submodule.span R (range BB))
  {l : {i // i < o} →₀ R}
  (hsupp : (l.support : set {i // i < o}) ⊆ {x | x.1 < i})
  (hl : finsupp.total {i // i < o} M R B l ∈ N)
  (hBB : ∀ i, ((B.repr (BB i)).support : set {i // i < o}) ⊆ {x | x.1 ≤ i})
...
begin
  ...
  have not_j_lt_x : ∀ (x : ords), ∀ (j : ordinal), x ∈ l.support → j < x.1 → j.succ ≠ i,
  { intros x j hx h_1 hj,
    have hi : j.succ ≤ x := succ_order.succ_le_of_lt h_1,
    rw hj at hi,
    have hi' := hsupp hx,
  ...
  obtain hgood | hgood := eq_or_ne (a ⟨j, hjo⟩) 0,
  { -- Case: j is not good
  ...
  { let r := Exists.some hc,
    ...
    { have hBBj := hBB j_good,
      ...
      exact hl'' hx },
    have hsupp' : (l'.support : set {i // i < o}) ⊆ {x | x.1 < j},
    { intros x hx,
      by_contradiction,
      have h : ¬ x.1 < j := h,
      have h := eq_or_lt_of_le (not_lt.1 h),
    ...
    { dsimp [l'],
      ...
      apply N.smul_mem' r,
      exact hBBN j_good },
    have h : finsupp.total {i // i < o} M R B l' ∈ submodule.span R (range BB) := H j hji hjo.le l' hsupp' hl',
    have hBBj : r • (BB j_good) ∈ submodule.span R (range BB) := submodule.smul_mem'
      (submodule.span R (range BB)) r (mem_of_subset_of_mem subset_span
        (⟨j_good, rfl⟩ : BB j_good ∈ range BB)),
    exact submodule.add_mem' (submodule.span R (range BB)) h hBBj },
  { refine H j hji hjo.le l (λ x hx, _) hl,
    obtain h | rfl | h := lt_trichotomy x.1 j,
    { exact h },
    { cases hjsupp (by simpa only [subtype.val_eq_coe, subtype.coe_eta]) },
    { cases not_j_lt_x x j hx h hj } }
end
...
lemma span_restrict (N : submodule R M) (s : set M) (hs : s ⊆ N.carrier) :
  ∀ x : N, x ∈ @submodule.span R N _ _ _ {x | N.carrier.restrict s x} ↔
  (x : M) ∈ submodule.span R s :=
begin
  simp only [submodule.mem_span],
  refine λ x, ⟨λ hx p hp, _, λ hx p hp, _⟩,
  { suffices : x ∈ comap N.subtype p,
    { simpa only [mem_comap, submodule.coe_subtype] using this },let s' : set N := N.carrier.restrict s,
    refine hx _ (λ y hy, _),
    exact hp hy },
  { suffices : ↑x ∈ map N.subtype p,
    { simpa only [mem_map, submodule.coe_subtype, set_like.coe_eq_coe,
      exists_eq_right] using this },
    refine hx _ (λ y hy, _),
    simp only [map_coe, submodule.coe_subtype, mem_image, set_like.mem_coe],
    refine ⟨⟨y, hs hy⟩, hp _, rfl⟩,
    change y ∈ s,
    exact hy }
end
...
lemma span_range_cod_restrict (N : submodule R M) {α : Type*} (f : α → M) (h : ∀ (x : α), f x ∈ N) :
  ∀ x : N, x ∈ @submodule.span R N _ _ _ (range (cod_restrict f N h)) ↔
    (x : M) ∈ submodule.span R (range f) :=
begin
...
theorem basis.reindex_apply_image {ι ι' : Type*} (b : basis ι R M) (e : ι ≃ ι') (s : set ι') :
...
lemma finsupp.support_single_add {α : Type*} {l : α →₀ M} {b : M} (hb : b ≠ 0) {i : α}
  (hi : i ∉ l.support) {s : set α} (h : ((finsupp.single i b + l).support : set α) ⊆ s) :
...
begin
  ...
  { have H : (finsupp.single i b + l) i ≠ 0 :=
    ...
    rw hx',
    exact h ((finsupp.mem_support_to_fun _ i).2 H) },
  { have H : (finsupp.single i b + l) x ≠ 0,
    ...
    exact h (finsupp.mem_support_iff.2 H) }
end

-- Lemma on supports of finsupps, useful for finsupp.induction.
lemma finsupp.mem_support_of_support_single_add {α : Type*} {l : α →₀ M} {b : M} (hb : b ≠ 0)
  {i : α} (hi : i ∉ l.support) {s : set α} (h : ((finsupp.single i b + l).support : set α) ⊆ s) :
  i ∈ s :=
begin
  haveI := classical.type_decidable_eq α,
  have H : (finsupp.single i b + l) i ≠ 0 := by simpa only [finsupp.not_mem_support_iff.1 hi,
    finsupp.coe_add, pi.add_apply, finsupp.single_eq_same, add_zero, ne.def],
  exact h (finsupp.mem_support_iff.2 H)
end

/-- A submodule of a free module over a principal ideal domain is free. -/
theorem submodule.nonempty_basis_of_pid' [is_domain R] [is_principal_ideal_ring R] {ι : Type u}
...
begin
  ...
  { intro i,
    ...
    exact_mod_cast this hi },

  have hcoordB : ∀ i : good, (B.coord i) (BB i) ≠ 0 := λ i, by rw hu'; exact i.2,

  have hBB' : ∀ i, (finsupp.support ((B.repr) (BB i)) : set ords) ⊆ {x | x.1 ≤ i},
  { intros i x hx,
    have h := hBB i,
  ...
  have hB_BB : ∀ i : ordinal, ∀ l : good →₀ R, (l.support : set good) ⊆ {x | x.1.1 < i} →
    ∃ lB : ords →₀ R, ((lB.support : set ords) ⊆ λ x, x.1 < i) ∧
      finsupp.total ords M R B lB = finsupp.total good N R BB l,
  { intros i l,
    apply @finsupp.induction good R _ _ l,
    { refine λ _, ⟨0, _⟩,
      simp only [finsupp.support_zero, finset.coe_empty, empty_subset, _root_.map_zero, coe_zero,
  ...
  { intros j hj,
    ...
    exact hj },
  refine ⟨good, basis.mk H₁ $ submodule.eq_top_iff'.2 $ λ x, _⟩,
  let l := B.repr (x : M),
...
instance submodule.free_of_pid_of_free [is_domain R] [is_principal_ideal_ring R] {N : submodule R M}
  [module.free R M] :
  module.free R N :=
begin
