...
{ hom := λ i j, if h : i ≤ a ∧ j ≤ a then begin
  ...
  comm := begin
    ...
    have aux₁ : a - ↑((complex_shape.down ℕ).next (a - i).nat_abs) = a - ↑((a - (i + 1)).nat_abs),
    { sorry },
    have aux₂ : a - ↑((complex_shape.down ℕ).prev (a - i).nat_abs) = a - ↑((a - (i - 1)).nat_abs),
    { sorry },
    rw [← X.X_d_eq_to_iso_assoc aux₁.symm, ← Y.X_eq_to_iso_d aux₂],
    conv_lhs { congr, skip, rw ← category.assoc },
    dsimp only [null_homotopic_of_projective_to_acyclic_aux],
    congr' 2; sorry
    -- { dsimp, apply int.coe_nat_inj, norm_num [← int.abs_eq_nat_abs],
