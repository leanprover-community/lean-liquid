...
/-- Construct a disccrete quotient from a locally constant function. -/
def lc_to_discrete_quotient (f : locally_constant X M) : discrete_quotient X :=
{ rel := λ a b, f a = f b,
...
/-- Construct a locally constant function on the discrete quotient assocaited to `f`. -/
def lc_desc (f : locally_constant X M) : locally_constant (lc_to_discrete_quotient f) M :=
{ to_fun := λ i, quotient.lift_on' i f (by tauto),
  is_locally_constant := λ U, is_open_discrete _ }

lemma lc_factors (f : locally_constant X M) : (lc_desc f) ∘ (lc_to_discrete_quotient f).proj = f :=
rfl
