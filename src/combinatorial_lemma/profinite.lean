...
S.fintype_diagram ⋙ Lbar.Fintype.Laurent_measures.{u u} r' ⋙ hom_functor r' Λ
...
(hom_functor r' Λ).map_cone
  (limit.cone (S.fintype_diagram ⋙ Lbar.Fintype.Laurent_measures.{u u} r'))
...
def hom_Lbar_cone_is_limit (c) : is_limit (((level r').obj c).map_cone
  (hom_Lbar_cone r' Λ S)) :=
begin
  let E := (limit.cone (S.fintype_diagram ⋙ Lbar.Fintype.Laurent_measures.{u u} r')),
