...
def Θ (S : Fintype.{u}) :
  (Fintype.Laurent_measures.{u} r ⋙ to_CompHausFiltPseuNormGrp₁.{u} r).obj S ⟶
...
def Θ_fintype_nat_trans :
  (Fintype.Laurent_measures.{u} r ⋙ to_CompHausFiltPseuNormGrp₁.{u} r) ⟶ (real_measures.functor.{u} p) :=
{ app := λ S, Θ p S,
