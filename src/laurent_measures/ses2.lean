...
def Θ (S : Fintype.{u}) :
  (Fintype_Laurent_measures.{u} r ⋙ to_CompHausFiltPseuNormGrp₁.{u} r).obj S ⟶
...
def Θ_fintype_nat_trans :
  (Fintype_Laurent_measures.{u} r ⋙ to_CompHausFiltPseuNormGrp₁.{u} r) ⟶ (real_measures.functor.{u} p) :=
{ app := λ S, Θ p S,
