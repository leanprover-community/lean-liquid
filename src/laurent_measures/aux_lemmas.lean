...
begin
  ...
  { rintro ⟨m, hm⟩,
    ...
    use ⟨m, hd⟩,
    dsimp only [R],
    simpa [mem_coe, mem_Icc, subtype.mk_le_mk, not_and, hd, forall_true_left,
      not_le, int.lt_iff_add_one_le, hm] },
  { rintro ⟨⟨x, hx⟩, h_mem⟩,
    dsimp only [R] at h_mem,
    simp only [subtype.mk_le_mk, coe_Icc, not_and, not_le, set.mem_Icc, hx, forall_true_left,
      int.lt_iff_add_one_le, set.mem_compl_iff] at h_mem,
    use ⟨x, h_mem⟩ },
...
begin
  ...
  { convert (sum_reverse (λ l, f (n - l) * 2 ^ l) m).symm using 1,
    ...
    rw [mem_range, nat.lt_succ_iff] at hx,
    rw [← zpow_coe_nat, int.coe_nat_sub hx],
    congr' 2,
    rw [h], ring, },
