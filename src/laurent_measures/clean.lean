...
/-
section group_div
variables {α : Type*} [group α] [preorder α] [covariant_class α α (*) (≤)] (N : α)

/--  If `α` is a group and `N : α` is an element, then the map `x ↦ x / N` is an
equivalence between `{x : α // N ≤ x}` and `{x : α // 1 ≤ x}`. -/
@[to_additive "If `α` is an additive group and `N : α` is an element, then the map `x ↦ x - N` is an
equivalence between `{x : α // N ≤ x}` and `{x : α // 0 ≤ x}`."]
def equiv_subtype_le_mul : {x // N ≤ x} ≃ {x : α // 1 ≤ x} :=
equiv.mul_inv N 1

@[simp, to_additive]
lemma equiv_subtype_le_mul_eval (x : {x : α // N ≤ x}) :
  ((equiv_subtype_le_mul N) x : α) = N⁻¹ * x :=
by { simp [equiv_subtype_le_mul] }

@[simp, to_additive]
lemma equiv_subtype_le_mul_symm_eval (x : {x : α // 1 ≤ x}) :
  ((equiv_subtype_le_mul N).symm x : α) = N * x :=
by { simp [equiv_subtype_le_mul] }
