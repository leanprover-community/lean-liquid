...
begin
  ...
  { let ε : ℝ≥0 := finset.min' (s₀.image $ nnnorm) (hs₀.image _),
    obtain ⟨a, hs₀, ha⟩ : ∃ a ∈ s₀, nnnorm a = ε,
    { rw ← finset.mem_image, apply finset.min'_mem },
    have H : 0 < ∥a∥ := by simpa only [norm_pos_iff] using finset.ne_of_mem_erase hs₀,
    have h0ε : 0 < ε, { simpa only [← ha] },
    have hε1 : ε ≤ 1,
    { replace hs₀ := finset.mem_of_mem_erase hs₀,
      simp only [set.finite.mem_to_finset, mem_filtration_iff, nnreal.coe_one] at hs₀,
      rwa [← ha] },
