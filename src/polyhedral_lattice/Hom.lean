...
noncomputable
def iso_of_equiv_of_strict (hf : function.bijective f)
...
@[simp]
lemma iso_of_equiv_of_strict.apply (x : M₁) (hf : function.bijective f)
  (strict : ∀ ⦃c x⦄, x ∈ filtration M₂ c → (equiv.of_bijective f hf).symm x ∈ filtration M₁ c) :
  (iso_of_equiv_of_strict f hf strict).hom x = f x := rfl

@[simp]
lemma iso_of_equiv_of_strict_symm.apply (x : M₂) (hf : function.bijective f)
  (strict : ∀ ⦃c x⦄, x ∈ filtration M₂ c → (equiv.of_bijective f hf).symm x ∈ filtration M₁ c) :
  (iso_of_equiv_of_strict f hf strict).symm.hom x = (equiv.of_bijective f hf).symm x := rfl
...
lemma HomZ_map_inverse_strict (r : ℝ≥0) [fact (0 < r)] [fact (r ≤ 1)] (M : Type)
  [profinitely_filtered_pseudo_normed_group_with_Tinv r M] :
  ∀ c f, f ∈ filtration ((Hom ℤ M)) c → (HomZ_map_equiv r M).symm f ∈ filtration M c :=
begin
  intros c f hf,
...
def HomZ_iso (r : ℝ≥0) [fact (0 < r)] [fact (r ≤ 1)] (M : Type)
  [profinitely_filtered_pseudo_normed_group_with_Tinv r M] :
  Hom ℤ M ≅ of r M :=
(iso_of_equiv_of_strict (HomZ_map r M)
  (HomZ_map_equiv r M).bijective (HomZ_map_inverse_strict r M)).symm
