...
def to_laurent_measures_nat_trans [fact (r' < 1)]:
  Fintype_invpoly r' ⟶ Fintype_Laurent_measures r' :=
...
@[simps]
def to_Lbar_nat_trans : Fintype_Laurent_measures r' ⟶ Lbar.Laurent_measures r' :=
{ app := λ S, to_Lbar_hom r' S,
  naturality' := λ S₁ S₂ f, begin
    ext,
    simp only [Fintype_Laurent_measures_map, category_theory.comp_apply, to_Lbar_hom_to_fun, to_Lbar_to_fun,
